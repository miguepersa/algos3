...
fun main(args: Array<String>) {
    try {
        ...
        var grafo: GrafoNoDirigidoCosto = GrafoNoDirigidoCosto(n)   // Representación de las calles de CaracasSur
        ...
        for (i in 1 until m+1) {
            ...
            grafo.agregarAristaCosto(AristaCosto(lad0[0].toInt(), lado[1].toInt(), lado[2].toInt()))
        }

        for (i in m+2 until fileContent.size) {
            var limpieza = fileContent[i].split(" ")
            grafo.listaLados[limpieza[0]].agregarLimpieza(limpieza[1].toInt(), limpieza[2].toInt())
        }

        var dijkstra = CCM_Dijkstra(grafo, casa)    // Ejecutamos Dijkstra en el grafo de CaracasSur
