...
public class CCM_Dijkstra(val g: GrafoDirigidoCosto, val s: Int) {
    ...
    init {
        ...
        while (!q.vacia()) {
            var u = q.extraerMinimo()
            verticesCamino.add(u)
            var ady = g.adyacentes(u.n)
            for (i in ady) {
                var v = arrVertices[i.y.n]
                if (v.key > u.key + i.costo) {  // Relajacion
                    q.decreaseKey(v.n, u.key + i.costo)
                    v.pred = u
                }
            }
        }
